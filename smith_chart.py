...
class SmithCanvas(FigureCanvas):
    ...
    # ── Quantum dual-trace overlay ────────────────────────────────────────────

    def plot_dual_trace(self, result_g0: dict, result_g1: dict) -> None:
        """
        Overlay two S11 traces on the Smith chart — one for qubit ground
        state |0⟩ and one for excited state |1⟩ — to visualise the
        dispersive shift as a separation between traces.

        The dispersive shift χ is extracted from the result dicts if the key
        ``chi_mhz`` is present; otherwise it is estimated from the frequency
        offset of the two resonance minima.

        Parameters
        ----------
        result_g0 : standard result dict for qubit in |0⟩ (ground state)
        result_g1 : standard result dict for qubit in |1⟩ (excited state)
                    Typically generated by shifting the readout frequency by χ.
        """
        self.ax.cla()
        self._draw_empty()       # background + Smith grid

        def _trace(res, color, label, linestyle="-"):
            ...
            if HAS_SKRF:
                try:
                    ...
                    self._fallback_trace_color(re, im, color)
            else:
                self._fallback_trace_color(re, im, color)

            # Mark resonance
            res_idx = int(np.argmin(res["s11_db"]))
            self.ax.plot(re[res_idx], im[res_idx], "*",
                         color=color, ms=12, zorder=9,
                         label=label)

        _trace(result_g0, C["teal"],  "|0⟩ ground state",  "-")
        _trace(result_g1, C["peach"], "|1⟩ excited state", "--")

        # Dispersive shift annotation
        chi = (result_g0.get("chi_mhz") or result_g1.get("chi_mhz"))
        if chi is None:
            # Estimate from resonance separation
            f0_res = float(result_g0["frequencies"][int(np.argmin(result_g0["s11_db"]))])
            f1_res = float(result_g1["frequencies"][int(np.argmin(result_g1["s11_db"]))])
            chi = abs(f1_res - f0_res) / 1e6
        ...
        self.ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.3",
                      facecolor=C["bg"], edgecolor=C["yellow"], alpha=0.9),
            zorder=12,
        )

        self.ax.legend(
            fontsize=6, facecolor=C["grid"],
            labelcolor=C["text"], edgecolor=C["border"],
            loc="lower left", framealpha=0.85,
        )
        self.fig.tight_layout(pad=1.4)
    ...
    def _fallback_trace_color(self, re, im, color):
        """Single-colour fallback polyline (used by plot_dual_trace)."""
        self.ax.plot(re, im, color=color, linewidth=2.0,
                     alpha=0.88, zorder=6)